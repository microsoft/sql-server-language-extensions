...
IF /I %BUILD_CONFIGURATION%==debug (
	cl.exe /MP /LD /Fe:nativecsharpextension.dll %DOTNET_NATIVE_SRC%\*.cpp /I %DOTNET_NATIVE_INCLUDE% /I %EXTENSION_HOST_INCLUDE% /D WINDOWS /D DEBUG /EHsc /Zi
) ELSE (
	cl.exe /MP /LD /Fe:nativecsharpextension.dll %DOTNET_NATIVE_SRC%\*.cpp /I %DOTNET_NATIVE_INCLUDE% /I %EXTENSION_HOST_INCLUDE% /D WINDOWS /EHsc /Zi
)
