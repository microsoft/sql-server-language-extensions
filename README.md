...
# Getting Started
Overview:
The directory \extension-host\ contains the extension API, while \language-extensions\ contains all of the extensions (i.e. language-extensions\java). All the scripts to trigger the builds of all extensions and projects are in \build\. The projects for testing framework are in \test\. Other linked repositories are in \submodules\.
...
3) Build mssql-java-lang-extension.jar
4) Signs the binaries
5) Packages a corresponding retail/debug zip of the signed binaries
6) Creates/publishes a new nuget which contains the following:
