...
//
// Purpose:
//  Utility functions implemented for the Windows OS.
//
//******************************************************************************************************
	...
	vector<char *> *tokens)
{
	...
	errno = 0;
	char *token = strtok_s(input, delimiter, &internalState);

	// Use strtok iteratively to do the tokenization.
	//
	while (errno == 0 && token != nullptr)
	{
		(*tokens).push_back(token);
		token = strtok_s(nullptr, delimiter, &internalState);
	}
