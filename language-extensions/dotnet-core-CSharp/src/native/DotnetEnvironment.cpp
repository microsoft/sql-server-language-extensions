...
void* DotnetEnvironment::load_library(const char_t *path)
{
    ...
    if (h == nullptr)
    {
        ...
        LOG_ERROR(std::string("Failed to load library: ") + (dl_error != nullptr ? dl_error : "unknown error"));
#endif
    }
    assert(h != nullptr);
    return (void*)h;
...
void* DotnetEnvironment::get_export(void *h, const char *name)
{
    ...
    {
#ifdef _WIN32
        LOG_ERROR(std::string("Failed to get export: ") + name);
#else
        const char *dl_error = dlerror();
        LOG_ERROR(std::string("Failed to get export '") + name + "': " + (dl_error != nullptr ? dl_error : "unknown error"));
#endif
    }
    assert(f != nullptr);
    return f;
